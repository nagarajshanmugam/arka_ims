<?php
   require('includes/common.php');
	...
	//include_once $config['SiteClassPath']."class.user_reg_window.php";
	...
	//$ObjEmployeeReg->RolesLists();
   // $ObjEmployeeReg->GetUserGroup();

	//$ObjCustomerReg->GetCustomerLists($objArray);
	if(isset($_GET['server_response']))
    {
    $objSmarty->assign('SuccessMessage', 'New Employee and Entitled Added Successfully');
    } 
	if(isset($_POST['hdAction']))
	{
    $ObjEmployeeReg->InsertEmployeeContent($_POST,$_FILES);
    }  
	
	///$objuser_reg_window= new user_reg_window();
 //   $objuser_reg_window->GetCLists($objArray);
	
    $objSmarty->assign('IncludeTpl', 'employe_reg.tpl');
