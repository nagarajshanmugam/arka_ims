<?php
   require('includes/common.php');
	include_once $config['SiteClassPath']."class.Staff.php";
   	include_once $config['SiteClassPath']."class.UserGroup.php";
	...
	//$ObjUserGroup->RolesLists();
	//$ObjUserGroup->GetUserGroupView($objArray);
	//$ObjCustomerReg->GetCustomerLists($objArray);
	if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage', 'New User Group and Entitled Added Successfully');
		 // header('Location:' ."customerDetails.tpl cust_id=".$_GET['cust_id']);
         } 
	if(isset($_POST['hdAction']))
  		$ObjUserGroup->InsertUserGroupContent($_POST);

	$ObjUserGroup->GetGroupLists($_REQUEST);
	$ObjUserGroup->GetRoles($_REQUEST);
		$ObjUserGroup->viewGetRoles($_REQUEST);
	  	if(isset($_GET['sm']))
	{ 
    ...
    $objSmarty->assign('IncludeTpl', 'userGroupReg.tpl');
