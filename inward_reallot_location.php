<?php
   require('includes/common.php');
	...
	$objStockInward->viewPutList($objArray);
    $objStockInward->list_inward_details();	
	if(isset($_POST['hdAction']))
		{
			$objStockInward->ReAllotLocation($_POST);
		}
	...
	if(isset($_GET['po_no']) || isset($_GET['invoice_no']))
		{
	$objStockInward->AjaxGetPoReAllotMaterialDetails($objArray);                  
		}
 
	if(isset($_GET['server_response']))
    {
        $objSmarty->assign('SuccessMessage1', 'Location Added Successfully');
    } 
	if(isset($_GET['locpo_no']) || isset($_GET['locinvoice_no']))
		{echo "hiiiiiiiii";
	$objStockInward->AjaxGetPoReAllotMaterialDetails($_GET);                  
		}
  
  $objSmarty->assign('IncludeTpl', 'inward_reallot_location.tpl');
