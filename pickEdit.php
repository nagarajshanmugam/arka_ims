<?php
 ...
 // include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	$objStaff->Roleslists();
  	$objpick = new Pick();
	$objpick->GetPickEdit();
	$objpick->list_Pic_details($objArray);
	$objpick->list_Picklevelid_details($objArray);
	if(isset($_GET['server_response']))
	 {
	  $objSmarty->assign('SuccessMessage1', ' Pick updated Successfully');
	 }
		 if(isset($_POST['hdAction']))
	 {    
  		$objpick->EditPick($_POST);
     }
 if(isset($_GET['serial_no']) && isset($_GET['mcode']))
  ...
  $objSmarty->assign('IncludeTpl', 'pickEdit.tpl');
