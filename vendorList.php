...
//	$objvendorReg->RoleList();
	//$objvendorList->GetvendorListView($_GET);
		$objvendorReg->GetVendorList($objArray);
		if(isset($_GET['supplier_code']))
	  {
		$objvendorReg->DeleteID($_GET['supplier_code']);		
       }
		//$objvendorReg->DeleteID($id);
	if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage1', 'Vendor Deleted Successfully');
  ...
  $objSmarty->assign('IncludeTpl', 'vendorList.tpl');
