<?php
  require('includes/common.php');
	...
	include_once $config['SiteClassPath']."class.productDetails.php";
...
//	$ObjproductDetails->RoleList();
	$objproductDetails->GetMaterialLists($objArray);
	
     if(isset($_GET['sl_no']) && isset($_GET['Pro'])){
	 	 $objproductDetails->DeleteID2($_GET);
	 }
	 
	 
  if(isset($_GET['server_success1']))
         {
          $objSmarty->assign('SuccessMessage1', 'Products Deleted Successfully');
		 // header('Location:' ."customerDetails.tpl cust_id=".$_GET['cust_id']);
         } 
  $objSmarty->assign('IncludeTpl', 'productDetailsList.tpl');
