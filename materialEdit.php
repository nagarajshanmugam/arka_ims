<?php
  ...
  include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	$objStaff->RolesLists($objArray);
	 ...
	 $objMaterialItem->GetMaterialDetails();
	//$objMaterialItem->RolesLists();
	//$objMaterialItem->GetMaterialItemView($_GET);
	
	if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage1', 'New Stock and Entitled Added Successfully');
		 // header('Location:' ."customerDetails.tpl cust_id=".$_GET['cust_id']);
         }
		 if(isset($_POST['hdAction']))
	{
  		$objMaterialItem->EditMaterial($_POST,$_FILES);
     }
 	  if(isset($_POST['hdAction']) && isset($_POST['Submit']))
	  {
	  
  	   $objMaterialItem->InsertStockContent($_POST);
      }
  		
		if(isset($_GET['deleid']))
		{
		$objMaterialItem->delete_inward_details($_REQUEST);
		}
  ...
  $objSmarty->assign('IncludeTpl', 'materialEdit.tpl');
