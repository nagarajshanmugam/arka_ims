<?php
  require('includes/common.php');
	...
	include_once $config['SiteClassPath']."class.StickerMgmt.php";
	...
	$objStickerMgmt	= new StickerMgmt();
	
	if(isset($_POST['hdAction']))
		{
			$objStickerMgmt->InsertStickerContent($_POST);
			

		}
	//$objOutward->RolesLists();
	$objOutward->lista_outward_details($objArray);
	
	/*$objOutward->GetStockOutwardView($objArray);
	$objOutward->list_outward_details($objArray);*/
	
	 if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage1', 'Outward Added Successfully');
         } 
  		$objOutward->GetOutwardDetail();
	

  $objSmarty->assign('IncludeTpl', 'outwardDetails.tpl');
