<?php
  require('includes/common.php');
	...
	$objStaff->Roleslists();
	  ...
	  {  
		$objputListReg->GetPutMasterDetails($_GET['sl_no']);	
		$objputListReg->GetPutMasterDetList($_GET['sl_no']);	
		
       }
	
	//$objpick->GetPickMasterDetails($objArray);
	$objputListReg->GetPutMasterList($objArray);
			if(isset($_GET['sl_no']) && isset($_GET['mcode']))
	  { 
				$objputListReg->DeleteIDEdit($_GET);	
       }
	       if(isset($_GET['server_responses']))
         {
          $objSmarty->assign('SuccessMessage1', 'Pick Deleted Successfully');
         }


  $objSmarty->assign('IncludeTpl', 'putDetails.tpl');
