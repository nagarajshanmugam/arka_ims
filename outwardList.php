...
//	$objSalesOrder	= new SalesOrder();
	if(isset($_GET['outward_entry_no'])){
	$objOutward->DeleteID($deleid);
	}
	$objOutward->GetOutwardList($objArray);
//    $objOutward->RolesLists();
	if(isset($_GET['server_response']))
	 ...
	 {    
  		$objOutward->AddPickListOutward($_POST);
     }

	/*//$objstockOutward->GetstockOutwardView($_GET);
	...
	if(isset($_GET['server_response']))
  	...
  	$objstockOutward->InsertStockContent($_POST);
		...
		{
		$objstockOutward->delete_inward_details($_REQUEST);
		}
	...
	$objSmarty->assign('SuccessMessage', $succs_msg_got);
	}			
	//$objStockOutward->list_inward_details();*/

  $objSmarty->assign('IncludeTpl', 'outwardList.tpl');
