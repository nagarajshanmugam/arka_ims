<?php
  ...
  include_once $config['SiteClassPath']."class.Outward.php";
  include_once $config['SiteClassPath']."class.stock_window.php";
  include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	$objOutward->GetOutwardDetail($objArray);
	
	/*$objstock_window	= new stock_window();
	$objstock_window->GetSLists($objArray);*/
	 if(isset($_GET['server_success']))
         {
          $objSmarty->assign('SuccessMessage', 'Outward Updated Successfully');
         } 
	if(isset($_POST['hdAction']))
	{
  		$objOutward->EditOutward($_POST);
     }
	 if(isset($_GET['outward_entry_no']) && isset($_GET['sl_no']) && isset($_GET['mcode']) && isset($_GET['qty']) )
	{
  		$objOutward->Delete($_GET);
     }
	  if(isset($_GET['server_responses']))
         {
          $objSmarty->assign('SuccessMessage1', 'Outward Deleted Successfully');
         }
	 $objOutward->list_outward_details();
	 if(isset($_GET['server_success1']))
         {
          $objSmarty->assign('SuccessMessage', 'Outward Deleted Successfully');
         }
  $objSmarty->assign('IncludeTpl', 'outwardEdit.tpl');
