<?php
   require('includes/common.php');
	include_once $config['SiteClassPath']."class.Staff.php";
   	include_once $config['SiteClassPath']."class.UserRole.php";
   ...
   $ObjUserRole->RolesLists();
   	if(isset($_GET['role_id']))
       $ObjUserRole->DeleteID($id);
//	$ObjUserRole->GetUserRoleView($objArray);
	//$ObjCustomerReg->GetCustomerLists($objArray);
   $ObjUserRole->GetUserRoleList();
	if(isset($_POST['hdAction']))
   $ObjUserRole->InsertUserRoleContent($_POST);
		
		
	  	if(isset($_GET['sm']))
   { 
   $msg_got=base64_decode($_GET['sm']);            
   $objSmarty->assign('SuccessMessage1', $msg_got);
   }	
	if(isset($_GET['em']))
   {  
   ...
   $objSmarty->assign('IncludeTpl', 'userRoleList.tpl');
