<?php
   require('includes/common.php');
	...
	$objStaff->Roleslists();
    $Objstock_mngmnt	= new stock_mngmnt();
$Objstock_mngmnt->GetStockDetails($objArray);
$Objstock_mngmnt->viewLevels($objArray);

	if(isset($_GET['server_error']))
         {
          $objSmarty->assign('ErrorMessage', 'Please Enter the Location Properly');
         } 
		 if(isset($_GET['server_response']))
    {
        $objSmarty->assign('SuccessMessage1', 'Stock Updated Successfully');
    } 
	if(isset($_POST['hdAction']))
	{
  		$Objstock_mngmnt->EditStockLocation($_POST);
     }
  $objSmarty->assign('IncludeTpl', 'stockEdit.tpl');
