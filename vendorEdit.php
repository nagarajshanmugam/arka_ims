<?php
  ...
  include_once $config['SiteClassPath']."class.vendorReg.php";
 // include_once $config['SiteClassPath']."class.split_page_results.php"; 
  	
	$objStaff		= new Staff();
	$objStaff->chkSLogin($objArray);
	$objStaff->RolesLists($objArray);
  	$objvendorReg	= new vendorReg();
//	$objvendorReg->RoleList();
	$objvendorReg->GetVendorDetails();
	//$objMaterialItem->GetMaterialItemView($_GET);
	
	if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage1', ' Vendor updated Successfully');
		 // header('Location:' ."customerDetails.tpl cust_id=".$_GET['cust_id']);
         }
		 if(isset($_POST['hdAction']))
	{
  		$objvendorReg->EditVendor($_POST);
     }
 	  /*if(isset($_POST['hdAction']) && isset($_POST['Submit']))
	  {
	  
  	$objvendorReg->InsertSupplierContent($_POST);
		...
		{
		$objvendorReg->delete_inward_details($_REQUEST);
		}
	...
	//$objMaterialItem->list_inward_details();

  $objSmarty->assign('IncludeTpl', 'vendorEdit.tpl');
