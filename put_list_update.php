<?php
  ...
  include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	if(isset($_POST['hdAction'])) 
	  {
  			$objputListReg->Put_List_Update($_POST);
      }
	/*$objSalesOrder->GetStockOutwardView($_GET);

	if(isset($_GET['oracle_so_no']))
    { 
	$objSalesOrder->list_so_details($_GET);
	}

	if(isset($_GET['server_response']))
	 {
	  $objSmarty->assign('SuccessMessage1', 'New Sales Order Added Successfully');
	 }
		 
 	  if(isset($_POST['hdAction'])) 
	  {
  			$objSalesOrder->InsertSoContent($_POST);
      }
  	if(isset($_GET['sl_no']) && isset($_GET['oracle_so_no'])&& isset($_GET['qty'])&& isset($_GET['mcode']))
     { 
		$objSalesOrder->DeleteAddDetail($_GET);	
     }*/
	 if(isset($_GET['server_responses']))
	 {
	    $objSmarty->assign('SuccessMessage1', 'SO Deleted Successfully');
	 }

		
  $objSmarty->assign('IncludeTpl', 'put_list_update.tpl');
