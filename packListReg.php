<?php
  ...
  include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	$objStaff->Roleslists();
  	$objpack	= new pack();
//	$objpack->GetStockOutwardView($_GET);
	//$objpack->GetSLists($objArray);
	//$objpack->list_inward_details();	
	if(isset($_GET['oracle_so_no']))
    { 
	$objpack->list_Pac_details($_GET);
	$objpack->list_Pac_master($_GET);
	}
$objpack->list_Pack_Slip_Id($_GET);

	if(isset($_GET['server_response']))
         {
          $objSmarty->assign('SuccessMessage1', 'Pack Details Updated Successfully');
         }
		 
 	  if(isset($_POST['hdAction'])) 
	  {
  			$objpack->InsertPackContent($_POST);
      }
	  if(isset($_POST['hdAction2'])) 
	  {
  			$objpack->InsertCarton($_POST);
      }
  		if(isset($_GET['outward_entry_no']) && isset($_GET['sl_no']))
		{
		$objpack->Delete($_GET['outward_entry_no'],$_GET['sl_no']);
		}
		if(isset($_GET['deleid']))
		{
		$objpack->delete_inward_details($_REQUEST);
		}
  ...
  $objSmarty->assign('IncludeTpl', 'packListReg.tpl');
