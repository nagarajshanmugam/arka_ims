<?php
 ...
 // include_once $config['SiteClassPath']."class.split_page_results.php"; 
	...
	$objputListReg->GetPutEdit();
	$objputListReg->list_Put_details($objArray);
	$objputListReg->list_Putlevelid_details($objArray);
	if(isset($_GET['server_response']))
	 {
	  $objSmarty->assign('SuccessMessage1', ' Put List updated Successfully');
	 }
		 if(isset($_POST['hdAction']))
	 {    echo "hiiiii";
  		$objputListReg->EditPut($_POST);
     }
 if(isset($_GET['serial_no']) && isset($_GET['mcode']))
	  { 
	    ...
	    if(isset($_GET['server_responses']))
         {
          $objSmarty->assign('SuccessMessage1', 'Put List Deleted Successfully');
         }
		

  $objSmarty->assign('IncludeTpl', 'putEdit.tpl');
