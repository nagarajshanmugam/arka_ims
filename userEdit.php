<?php
   require('includes/common.php');
    ...
    $ObjUsers	= new Users();
	$ObjUsers->RolesLists();
    $ObjUsers->GetUserDetails();
	$ObjUsers->GetUserGroup();

	//$ObjCustomerReg->GetCustomerLists($objArray);
	if(isset($_GET['server_success']))
    {
    $objSmarty->assign('SuccessMessage', 'Users Edited Successfully');
   // header('Location:'."user_details_view.php?user_id=".$_GET['user_id']);
    } 
	if(isset($_POST['hdAction']))
	{
    $ObjUsers->EditUser($_POST);
  	}
    $objSmarty->assign('IncludeTpl', 'userEdit.tpl');
